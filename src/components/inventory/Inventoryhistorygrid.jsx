...
import { useBeforeunload } from 'react';
...
import Inventoryitemhistorygrid from './Inventoryitemhistorygrid';
      ...
      <TableRow>
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={8}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Inventoryitemhistorygrid items={row.items}/>
          </Collapse>
        </TableCell>
      </TableRow>
    </React.Fragment>
        ...
        <TableCell />
          ...
          {headCells.map((headCell) => (
            <TableCell
              ...
              //text-align={headCell.numeric ? 'right' : 'left'}
...
export default function CollapsibleTable() {
  ...
  const REQUEST_URL="/Inventory/GetOrdersHistory";
  ...
  //let rows = [];
  //useSelector(() => store.getState().orderitem.ApprovalOrders);
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar numSelected={selected.length} />
          ...
          <TextField id="deptFilter" label="Department" variant="standard" style={{display :  approverRoles.includes(auth?.roles)? 'block' : 'none'}} onChange={handleFilter} />
